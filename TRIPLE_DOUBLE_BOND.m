...
function new_chm = TRIPLE_DOUBLE_BOND(chm)

%% TRIPLE_DOUBLE_BOND  changes bond type from triple to double bond
...
while attempt<max_tries && success==0
    ...
    if random_atom>1
        ...
        if chm(atom_bond_change_idx-1)=="#"
            ...
            new_chm(atom_bond_change_idx-1)="=";
            new_chm(atom_bond_change_idx+1:chm_len)=chm(atom_bond_change_idx+1:chm_len);
        end
    end
