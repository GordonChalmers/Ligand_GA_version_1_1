...
function new_chm = SINGLE_TRIPLE_BOND(chm)

%% SINGLE_TRIPLE_BOND  changes bond type from single to triple
...
while attempt<max_tries && success==0
    ...
    if random_atom>1
        ...
        if avail_heavy_bond(random_atom)>=2 && avail_heavy_bond(min_conect_atom)>=2 && chm(atom_bond_change_idx-1)~="=" && chm(atom_bond_change_idx-1)~="#"
    ...
    %% pdb wasn't created - shouldn't happen
    if exist(ligand_dir+"/molecule/molecule.001.pdb")==0
        success=0;
    end
