...
function new_chm = DOUBLE_TRIPLE_BOND(chm)

%% DOUBLE_TRIPLE_BOND  changes bond type from double to triple bond
...
while attempt<max_tries && success==0
    ...
    if random_atom>1
        ...
        if avail_heavy_bond(random_atom)>=1 && avail_heavy_bond(min_conect_atom)>=1 && chm(atom_bond_change_idx-1)=="="
