...
function penalty=ADME_Penalty_Multi_Protein(molecule_idx,chm,heavy_idx_chm,avail_heavy_bond)
...
%% parse log.out file if gold output has it (check write_options to write files) and if needed
%% log_file=ligand_dir+"/molecule"+molecule_idx+"/stereoisomer"+fileno+"_"+Protein_Name(protein_idx)+"/docked_molecule_file"+fileno+".mol2";
...
if strcmp(ADME_Penalty_Type,'HardLipinski')==1
    ...
    %% Lipinski hydrogen bond donors (NH or OH)
    total_donors=total_donors-3;  %% take into account a blank line and a line of text
    if total_donors>max_donors_ADME
        violations=violations+1;
    end
